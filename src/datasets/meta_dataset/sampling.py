...
class EpisodeDescriptionSampler(object):
    ...
    def __init__(self,  # noqa: E111
        ...
        if self.n_tasks > 0:
            if tasks_path is None:
                print(f'Generating {n_tasks} class_ids')
                self.class_ids = [self.sample_class_ids() for _ in range(self.n_tasks)]
            else:
                print(f'Loading {n_tasks} class_ids from {tasks_path}')
                self.class_ids = torch.load(tasks_path)
    
    def sample_fixed_n_class_ids(self):
        ids_rel = [  # noqa: E111
            class_id - self.class_set[0] for class_id in self._filtered_class_set
        ]
        episode_classes_rel = sample_class_ids_uniformly(self.num_ways, ids_rel)
    ...
    def sample_episode_description(self, task_idx=None):  # noqa: E111
        ...
        """
        if self.n_tasks > 0 and task_idx is not None:
            class_ids = self.class_ids[task_idx]
        elif self.num_ways:
            class_ids = self.sample_fixed_n_class_ids()
        else:
