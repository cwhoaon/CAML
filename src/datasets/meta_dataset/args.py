...
def get_args_parser():
    ...
    parser.add_argument('--ada_lrs', default=None, type=float, nargs='+', help='Learning rates for feature adaptation steps')
    ...
    parser.add_argument('--resume_flow', default='', help='resume flow from checkpoint')
    ...
    # lora parameters
    parser.add_argument('--lora_rank', default=0, type=int, help='Rank of the LoRA approximation')
    parser.add_argument('--shared_A', action='store_true', help='Use shared A for all tasks')
    parser.add_argument('--n_basis', default=0, type=int, help='Number of basis for LoRA')

    # number of tasks
    parser.add_argument('--n_tasks', default=0, type=int, help='Number of tasks for multi-task LoRA')
    parser.add_argument('--tasks_path', default=None, type=str, help='Path to tasks file')

    # evaluate train dataset
    parser.add_argument('--evaluate_train', action='store_true', help='Evaluate training dataset')

    # evaluate valid dataset
    parser.add_argument('--evaluate_valid', action='store_true', help='Evaluate validation dataset')

    # gaudi option
    parser.add_argument('--gaudi', action='store_true', help='Enable when running on Gaudi GPUs')

    # skip first eval option
    parser.add_argument('--skip_first_eval', '-sfe', action='store_true', help='Disable first evaluation')

    # ignore task index
    parser.add_argument('--ignore_task_index', '-iti', action='store_true', help='Ignore task index in the dataset')

    # save postfix
    parser.add_argument('--save_postfix', '-sptf', default='', type=str, help='Postfix to append to the save path')

    # bias tuning
    parser.add_argument('--bitfit', action='store_true', help='Enable task-specific bias tuning during meta-training')
    parser.add_argument('--bias_tuning', action='store_true', help='Enable bias tuning')
    parser.add_argument('--fc2_bias_only', action='store_true', help='Only tune the bias of the fc2 layers')
    parser.add_argument('--qkv_bias_only', action='store_true', help='Only tune the bias of the qkv layers')

    # save fine-tuning trajectory
    parser.add_argument('--save_fine_tuning_trajectory', '-sftt', action='store_true', help='Save fine-tuning trajectory')
    parser.add_argument('--save_last_only', '-sfo', action='store_true', help='Save only the last fine-tuned parameters')

    # basis params path
    parser.add_argument('--basis_params_path', default=None, type=str, help='Path to basis parameters')

    # apply data/class/task transform
    parser.add_argument('--apply_data_transform', '-adt', action='store_true', help='Apply data transform during fine-tuning')
    parser.add_argument('--apply_class_transform', '-act', action='store_true', help='Apply class transform during fine-tuning')
    parser.add_argument('--apply_task_transform', '-att', action='store_true', help='Apply task transform during fine-tuning')

    # flow params
    parser.add_argument('--flow_hidden_dim', default=384, type=int, help='Hidden dimension of the flow')
    parser.add_argument('--attentive_flow', '-af', action='store_true', help='Load attentive flow')
    parser.add_argument('--flow_drift_v2', '-fv2', action='store_true', help='Load flow drift v2')
