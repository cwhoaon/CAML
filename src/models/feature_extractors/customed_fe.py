...
class BitfitBias(nn.Module):
    def __init__(self, dim, n_bias=1):
        super().__init__()
        self.dim = dim
        self.n_bias = n_bias
        self.q_bias = nn.Parameter(torch.zeros(n_bias, dim))
        self.k_bias = nn.Parameter(torch.zeros(n_bias, dim))
        self.v_bias = nn.Parameter(torch.zeros(n_bias, dim))
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        self.qkv = nn.Linear(dim, dim * 3, bias=False) # bias False
        self.qkv_bias = BitfitBias(dim, n_bias) # My custom
    ...
    def forward(self, x: torch.Tensor, bias_idx) -> torch.Tensor:
        B, N, C = x.shape
        qkv = self.qkv_bias(self.qkv(x), bias_idx).reshape(B, N, 3, self.num_heads, self.head_dim).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, bias_idx) -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x), bias_idx)))
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x: torch.Tensor, bias_idx) -> torch.Tensor:
        x = self.patch_embed(x)
        x = self._pos_embed(x)
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            x, bias_idx = self.blocks(x, bias_idx)
        x = self.norm(x)
        return x
    ...
    def forward(self, x: torch.Tensor, bias_idx) -> torch.Tensor:
        x = self.forward_features(x, bias_idx)
...
if __name__=='__main__':
    ...
    inp = torch.randn((batch_size, n_s+n_q, 3, 224, 224))
    inp = inp.reshape(-1, 3, 224, 224)
    y_s = torch.randint(0, 6, (25,))
    bias_idx = torch.arange(batch_size)
    bias_idx = torch.repeat_interleave(bias_idx, n_s+n_q)
    output = vit(inp.to('cuda'), bias_idx.to('cuda'))
    
    print(inp.shape, y_s.shape, bias_idx)
    print(output.shape)